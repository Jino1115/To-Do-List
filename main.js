...
submit.onclick = function () {
    if (input.value !== "") {
        if (isHTML(input.value)) {
            ...
            if (submit.value === "update") { //I toggle between them whenever I have update or add 
...
todoListDiv.addEventListener("click", (event) => {
    ...
    if (event.target.classList.contains("todo-item")) { ///First if we checked it
    ...
    else if (event.target.classList.contains("todo-item-done")) { ///If we unchecked the item
        let todoItemElement = event.target; // get the todo item element
        let id = todoItemElement.getAttribute("data-id"); // get the id of the todo item element
    ...
    else if (event.target.classList.contains("todo-text")) {   ///Same
    ...
    else if (event.target.classList.contains("todo-title")) { ///Same
...
function toggleTodoStatusById(clickedCheckbox) {
    let id = clickedCheckbox.getAttribute("id").split("-")[3]; //The checkbox’s id is formatted like "todo-checkbox-id-1234", and split("-")[3] gets the actual numeric part (the id of the to-do item).
